...
func validateGitHubParams(owner, repo, path, token string) error {
	if owner == "" {
		return errors.New("missing GitHub storage environment variable: GITHUB_OWNER")
	}
	if repo == "" {
		return errors.New("missing GitHub storage environment variable: GITHUB_REPO")
	}
	if path == "" {
		return errors.New("missing GitHub storage environment variable: GITHUB_FILE_PATH")
	}
	if token == "" {
		return errors.New("missing GitHub storage environment variable: GITHUB_TOKEN")
	}
	return nil
}
...
func ReadAllItems(owner, repo, path, token string) ([]map[string]interface{}, error) {
	if err := validateGitHubParams(owner, repo, path, token); err != nil {
		return nil, err
	}
